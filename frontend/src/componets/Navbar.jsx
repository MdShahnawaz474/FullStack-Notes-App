...
const [userData, setUserData]= useState(null)
  ...
  return (
    ...
    <div onClick={()=>{
      navigate("/")
    }} className='Logo text-2xl cursor-pointer'>Keep  <span className="text-[#578DF7]"> Notes</span> </div>

    <div className='right flex items-center gap-[12px]'>
      <ul className='flex items-center gap-3'> 
      <Link to={"/"}><li className='text-xl '>Home</li></Link>
      <li className='text-xl font-normal'>Online Status :{onlineStatus ? "✅" : "🔴"} </li>
      </ul>

      <button className='btnNormal rounded-full' onClick={()=>{
       navigate("/addNewNote")
      }} >Add Note</button>
      <Avatar onClick={()=>{navigate("/profile")}} name={userData?userData.username:""} size='50' className='rounded-[50%] cursor-pointer'/>
